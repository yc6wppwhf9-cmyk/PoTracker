...
def notify_grn_received(
    ...
    *,
    ...
    """Tell the approver that goods have arrived.

    Sent once per import rather than per receipt: a register can match hundreds
    of lines at a time, and that volume would train people to ignore it.

    The mail deliberately carries counts and a handful of examples, not the
    whole register. Deciding anything from it means opening the GRN register,
    where the pending, complete and excess quantities are shown against the
    order — the mail exists to say "something arrived, go and look".
    """
    if receipts <= 0:
        return {"sent": False, "skipped": True, "reason": "no receipts imported"}

    parts = [
        f"<p><strong>{receipts:,}</strong> receipt line(s) were imported from "
        f"the GRN register, of which <strong>{matched:,}</strong> matched a "
        "purchase order.</p>"
    ]

    if completed:
        items = "".join(
            "<li><strong>{po}</strong> — {n} line(s) now fully received</li>".format(
                po=html.escape(str(c.get("po_number") or "—")),
                n=c.get("lines", 0),
            )
            for c in completed[:5]
        )
        more = (
            f"<li>…and {len(completed) - 5} more</li>" if len(completed) > 5 else ""
        )
        parts.append(
            f"<p><strong>{len(completed)}</strong> purchase order(s) are now "
            f"complete:</p><ul>{items}{more}</ul>"
        )

    if over:
        items = "".join(
            "<li><strong>{po}</strong> {code} — ordered {o:,.0f}, received "
            "{r:,.0f} (<strong>+{e:,.0f}</strong>)</li>".format(
                po=html.escape(str(o.get("po_number") or "—")),
                code=html.escape(str(o.get("item_code") or "—")),
                o=float(o.get("ordered") or 0),
                r=float(o.get("received") or 0),
                e=float(o.get("excess") or 0),
            )
            for o in over[:5]
        )
        more = f"<li>…and {len(over) - 5} more</li>" if len(over) > 5 else ""
        parts.append(
            '<p style="color:#b91c1c"><strong>'
            f"{len(over)}</strong> line(s) received MORE than the purchase "
            f"order asked for:</p><ul>{items}{more}</ul>"
        )

    parts.append(
        _button(app_url("/procurement/receipts"), "Open the GRN register")
    )

    return send_email(
        emails_with_role(client, "approver"),
        f"{receipts:,} receipt(s) imported"
        + (f" — {len(over)} over-received" if over else ""),
        "".join(parts),
    )
